...
def get_select(one_dict):
    ...
    select_sql=('select reportCode from T_ReportStock  where '
                ...
                '= (SELECT code FROM T_ResearchFellow where name'
...
#    print(prim_key)
    ...
    if len(select_result) ==1:
        ...
        for year in ['2016','2017','2018']:
            ...
            '''
            # 进行更新的数据
            indata = [yjbgbm,one_item.get('code'),'{}-12-31'.format(year),
                    one_item.get('{}Ezyywsr'.format(year)),
                    one_item.get('{}Esyl'.format(year)),
                    one_item.get('{}Egsmgsjlr'.format(year)),
                    one_item.get('{}Emgsy'.format(year))]            
            
            # 判断是否插入还是更新
            select1 = ('select * from  T_FinancialPredict where REPORTCODE='
                       ' \'{}\'  and PERIODDATE=\'{}\''.format(yjbgbm,
                       '{}-12-31'.format(year)))
            cour1.execute(select1)
            jieguo1 = cour1.fetchall()
            if len(jieguo1) ==1:
                print('{code} {year} {name} 需要更新 '.format(code=one_item.get('code'),
                                     year=year,
                                     name=one_item.get('research')))
                ...
                print('{code} {year} {name} 需要插入 '.format(code=one_item.get('code'),
                                     year=year,name=one_item.get('research')))
                ...
                print('{code} {year} {name} 存在多个报告编码 '.format(code=one_item.get('code'),
                                     year=year,name=one_item.get('research')))
        ...
        print('{code} {name} 数据有多条记录'.format(code=one_item.get('code'),
                                     name=one_item.get('research')))
